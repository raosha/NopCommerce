...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public virtual IPagedList<Customer> GetAllCustomers(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, bool getOnlyActiveCustomers = false)
        {
        ...
        public List<Customer> GetCustomersByRole(string roleName)
        {
            var customers = new List<Customer>();
            
            var roleByName = _customerRoleRepository.Table.FirstOrDefault(x => x.Name == roleName);

            if (roleByName != null)
            {
                var roleMapping =
                    _customerCustomerRoleMappingRepository.Table.Where(x => x.CustomerRoleId == roleByName.Id);

                if (roleMapping != null && roleMapping.Any())
                {
                    foreach (var customerCustomerRoleMapping in roleMapping)
                    {
                        var customer =
                            _customerRepository.Table.FirstOrDefault(
                                x => x.Id == customerCustomerRoleMapping.CustomerId);
